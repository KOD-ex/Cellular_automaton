...
var game_state = [];
var connections = [];
var b, s, stop, d, input;
var count_cell;
var len_cell;

input = document.getElementById('count_cell');
input.oninput = function() {
	count_cell = $("#count_cell").val();
	len_cell = 600/count_cell;
	...
	for (var i=0; i<count_cell; i++){
		for (var j=0; j<count_cell; j++){
			if (connections[i][j]<s[0] || connections[i][j] > s[s.length-1]) if (game_state[i][j] == 1){
				game_state[i][j]=0;
				d.clearRect(i*len_cell,j*len_cell);
			}

			if (connections[i][j] == b[0]) if (game_state[i][j] == 0){
				game_state[i][j]=1;
				d.drawRect(i*len_cell,j*len_cell,"black");
			}
...
canvas.onclick = function(event){
	...
	x_rect = Math.floor(x_rect/len_cell); 
	y_rect = Math.floor(y_rect/len_cell); 

	game_state[x_rect][y_rect] = 1;
	d.drawRect(x_rect * len_cell, y_rect * len_cell, 'black');
}
